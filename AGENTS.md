...
### Layer 1: the declared tags

#### Declared sizes are deduplicated by URL

Core generates only four Site Icon derivatives — 270, 192, 180, 32 — and resolves anything else to the smallest one at least as large. Without an image service, 120/152/167/180 all return the same 180x180 file, and declaring all four would claim four sizes for one image. `get_declarable_icons()` collapses them.

### Layer 2: routing and the response

#### Path matching on `init:100`, not a rewrite rule

Rewrite rules need a flush to take effect and a deploy cannot be relied on to perform one. Priority 100 is late enough for CDN and media URL filters to have registered, early enough to skip the main query.

#### `-precomposed` goes on either side

iOS probes `apple-touch-icon-152x152-precomposed.png` *first*, then the same name without the suffix, then `apple-touch-icon-precomposed.png`, then the bare one — dimensions before the suffix. `TOUCH_ICON_PATTERN` and `nginx.conf.example` originally accepted only `-precomposed-152x152`, a shape nothing requests, so the highest-priority filename fell through to a 404 while the lower-priority ones worked. That is why it went unnoticed: a client that walks the whole list still gets served, and the test asserted the invented order rather than Apple's. The PHP pattern uses a branch reset — `(?|` — so all three orders number their dimensions 1 and 2 and `resolve_touch_icon_size()` stays positional; `-precomposed` on *both* sides is deliberately not matched, since it is not a variant anything asks for. nginx does not capture and so is written loosely, matching the double form and leaving PHP to refuse it, exactly as nginx already allows any `\d+x\d+` and lets `SUPPORTED_SIZES` decide.

#### Serve bytes, don't redirect

Nothing guarantees an icon fetcher follows a redirect, and those fetchers are the entire audience. Redirecting remains available via `site_icon_fallback_serve_mode` and is the fallback when fetching bytes fails.

#### Content and redirects get different cache lifetimes

Bytes are content and can be cached hard — a Site Icon change also changes the URL, so stale bytes cannot surface under a live address. A redirect is a *pointer*, and a Site Icon change deletes what it points at, so a long-cached redirect leaves clients replaying it into a 404. This was a real shipped bug; don't collapse the two.
...
**Some hosts pin `/favicon.ico` with an exact-match location.** Altis is one, and an exact match cannot be outvoted by a regex, so the snippet's `favicon` location never sees that path. Reaching it takes the rewrite: see "`/favicon.ico` is reached by a rewrite, not a location."

**The snippet needs `root` declared at server level.** Both halves of the file-wins guarantee resolve a path against whatever `root` is in scope: `try_files $uri` in the location blocks, and `$request_filename` in the rewrite guard, which is evaluated before any location has been selected. A config that declares `root` only inside individual locations resolves both against nginx's default docroot instead, and a real `favicon.ico` at the web root stops being found. The rewrite did not introduce this; the location blocks always depended on it.
...
- Length is the symptom, not the rule. Past roughly 200 lines, look for the seam — but don't cut where there isn't one. `icon-fetch.php` is 332 lines of a single concern, and pulling `ALLOWED_TYPES` out would only separate the allow-list from its two callers.
